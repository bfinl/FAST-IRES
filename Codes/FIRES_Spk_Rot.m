...
% Other nEDED vARIABLES, PARAMETERS, etc.
SPK = 'Spike_I';
Fig_Folder = ['../Figures/',SPK];
...
SNR_avg  = mean(std(Sigma_inv_half*Phi_noisy(:,1:end),[],2).^2)/mean(std(Sigma_inv_half*Noise_only(:,Noise_st:Noise_end),[],2).^2)
D_W      = ((norms(K+10^-20).^-1));
...
% alpha_vec = [0.5 0.25 0.2 0.1 0.05 0.025 0.01 0.075 0.005 0.0025 0.001 0.0005 0.0001]';
alpha_vec = [0.08];
